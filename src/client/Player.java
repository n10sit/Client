...
public final class Player extends Entity {
	...
	public final String titleNpc(final int i) {
		switch (i) {
			case 0:
			return "Noob (PvN) ";
			case 1:
	...
	public final String titlePvp(final int i) {
		switch (i) {
			case 0:
			return "Noob (PvP) ";
			case 1:
	...
	public final String titleColor(final int i) {
		if (titleToUse(i).contains("Noob")) {
			return "<col=00ff00>";
		} else if (titleToUse(i).contains("Novice")) {
			return "<col=ffff00>";
		} else if (titleToUse(i).contains("Veteran")) {
			return "<col=00ffff>";
		} else if (titleToUse(i).contains("Renowned")) {
			return "<col=0000ff>";
		} else if (titleToUse(i).contains("Hero")) {
			return "<col=ff0000>";
		} else if (titleToUse(i).contains("Legend")) {
			return "<col=ff7000>";
		} else {
			return "";
		}
	...
	public final String titleColor2(final int i) { //gre yel cya blu red or2
		if (titleToUse(i).contains("Noob")) {
			return "@gre@";
		} else if (titleToUse(i).contains("Novice")) {
			return "@yel@";
	...
	public final String titleToUse(final int i) {
		if (rankForKillsNPC(npcKills) > rankForKillsPVP(pvpKills)) {
			return titleNpc(rankForKillsNPC(npcKills));
		} else {
			return titlePvp(rankForKillsPVP(pvpKills));
		}
	}

	public int bestKills() {
		if (npcKills > pvpKills) {
			return npcKills;
		} else {
			return pvpKills;
		}
	...
	public int rankForKillsNPC(final int i) {
		if (i >= 10 && i < 50) {
			return 1;
		} else if (i >= 50 && i < 200) {
			return 2;
	...
	public int rankForKillsPVP(final int i) {
		if (i >= 5 && i < 25) {
			return 1;
		} else if (i >= 25 && i < 100) {
			return 2;
	...
	public void updatePlayer(Stream stream)
	{
		...
		npcKills = stream.readUnsignedWord();
		pvpKills = stream.readUnsignedWord();
		monsterLevel = stream.readUnsignedWord();
		visible = true;
		aLong1718 = 0L;
