...
public class client extends RSApplet {
	...
	public void drawChannelButtons() {
		...
		smallText.method389(true, 425, 0xffffff, "Open Bank", 157);
	...
	private void drawChatArea() {
		...
		if (messagePromptRaised) {
				...
				if (chatMessages[k] != null) {
					...
					if (chatType == 0) {
						if (chatTypeView == 5 || chatTypeView == 0) {
							if (yPos > 0 && yPos < 210)
								// textDrawingArea.method389(false, 11, 0, chatMessages[k], yPos);//chat color
								// enabled
					...
					if (chatType == 6 && (splitPrivateChat == 0 || chatTypeView == 2) && privateChatMode < 2) {
						if (chatTypeView == 2 || chatTypeView == 0) {
							if (yPos > 0 && yPos < 210) {
								...
								// textDrawingArea.method385(0x800000, chatMessages[k], yPos, 15 +
						...
						if (chatType == 11 && (clanChatMode == 0)) {
							...
							if (chatType == 12) {
								if (yPos > 0 && yPos < 110)
									...
									// 11);
									newRegularFont.drawBasicString(chatMessages[k] + " <col=255>" + s1, 11, yPos,
											0x7e3200, -1);
								j++;
							}
						}
			...
			if (myPlayer != null && myPlayer.name != null)
				s = myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(myPlayer.bestKills()) + "</col>"
						+ myPlayer.name;
			// s = "<col=00ff00>"+myPlayer.titleToUse(myPlayer.bestKills())+"</col>" +
			// myPlayer.name;
			else
				s = myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(myPlayer.bestKills()) + "</col>"
						+ myPlayer.name;

			String textForWidth = myPlayer.titleToUse(myPlayer.bestKills()) + myPlayer.name;
			// textDrawingArea.method385(0, s + ":", 133, 11);
			newRegularFont.drawBasicString(s + ":", 11, 133, 0, -1);
			// textDrawingArea.drawChatInput(255, 12 + textDrawingArea.getTextWidth(s + ":
			// "), inputString + "*", 133, false); //original star
			// textDrawingArea.method389(false, 12 + textDrawingArea.getTextWidth(s + ": "),
			// 255, inputString + ((loopCycle % 40 < 20) ? "@bla@|" : ""), 133); //flashing
			newRegularFont.drawBasicString(inputString + ((loopCycle % 40 < 20) ? "|" : ""),
					12 + textDrawingArea.getTextWidth(textForWidth + ": "), 133, 255, -1);
	...
	public void init() {
		...
		setHighMem();
		isMembers = true;
		initClientFrame(503, 765); // 503 765
	}
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
		...
		if (super.clickMode3 == 1) {
			if (super.saveClickX >= 5 && super.saveClickX <= 61 && super.saveClickY >= 482 && super.saveClickY <= 505) {
				...
				if (openInterfaceID == -1) {
					...
					pushMessage("Please close the interface you have open before opening the bank.'", 0, "");
				}
			}
		}
	}
	...
	private void loadTitleScreen() {
		...
		drawLoadingText(10, "Loading Titlescreen...");
		if (!aBoolean831) {
			drawFlames = true;
			aBoolean831 = true;
	...
	private void loadingStages() {
		if (lowMem && loadingStage == 2 && ObjectManager.anInt131 != plane) {
			aRSImageProducer_1165.initDrawingArea();
			aTextDrawingArea_1271.drawText(0, "Infinitum", 151, 257);
	...
	void drawLoadingText(int i, String s) {
		...
		char c1 = '\310';
		byte byte1 = 20;
		chatTextDrawingArea.drawText(0xffffff, "Infinitum", c1 / 2 - 26 - byte1, c / 2);
		int j = c1 / 2 - 18 - byte1;
				...
				if (openInterfaceID == -1) {
					...
					for (int i3 = 0; i3 < RSInterface.interfaceCache.length; i3++) {
						if (RSInterface.interfaceCache[i3] == null || RSInterface.interfaceCache[i3].contentType != 600)
							continue;
						reportAbuseInterfaceID = openInterfaceID = erface.interfaceCache[i3].parentID;
						break;
					}
				...
				if ((j == 13 || j == 10) && inputString.length() > 0) {
					if (myPrivilege == 2 || server.equals("127.0.0.1")) {
						...
						if (inputString.equals("::hi"))
							pushMessage("@gre@h@blu@e@or2@l@red@l@cya@o", 0, "");
					...
					if (inputString.startsWith("getitemints")) {
						String[] args = inputString.split(" ");
						int item = Integer.parseInt(args[1]);
						System.out.println("ints for item id: " + item);
						System.out.println("model id: " + ItemDef.forID(item).modelID);
						System.out.println("model zoom: " + ItemDef.forID(item).modelZoom);
						System.out.println("model off1: " + ItemDef.forID(item).modelOffset1);
						System.out.println("model off2: " + ItemDef.forID(item).modelOffset2);
						System.out.println("model rot1: " + ItemDef.forID(item).modelRotation1);
						System.out.println("model rot2: " + ItemDef.forID(item).modelRotation2);
						System.out.println("model male equip: " + ItemDef.forID(item).maleEquip1);
						System.out.println("model female equip: " + ItemDef.forID(item).femaleEquip1);
						System.out.println("model male equip 2: " + ItemDef.forID(item).maleEquip2);
						System.out.println("model female equip 2: " + ItemDef.forID(item).femaleEquip2);
						System.out.println("abyte205: " + ItemDef.forID(item).aByte205);
						System.out.println("abyte154: " + ItemDef.forID(item).aByte154);
						System.out.println("anint188: " + ItemDef.forID(item).anInt188);
						System.out.println("anint164: " + ItemDef.forID(item).anInt164);
						System.out.println("anint175: " + ItemDef.forID(item).anInt175);
						System.out.println("anint204: " + ItemDef.forID(item).anInt204);
						System.out.println("team: " + ItemDef.forID(item).team);
						System.out.println("value: " + ItemDef.forID(item).value);
						System.out.println("anint162: " + ItemDef.forID(item).anInt162);
						System.out.println("anint166: " + ItemDef.forID(item).anInt166);
						System.out.println("anint167: " + ItemDef.forID(item).anInt167);
						System.out.println("anint173: " + ItemDef.forID(item).anInt173);
						System.out.println("anint184: " + ItemDef.forID(item).anInt184);
						System.out.println("anint185: " + ItemDef.forID(item).anInt185);
						System.out.println("anint188: " + ItemDef.forID(item).anInt188);
						System.out.println("anint191: " + ItemDef.forID(item).anInt191);
					...
					if (inputString.startsWith("getnpcanims")) {
						String[] args = inputString.split(" ");
					...
					if (inputString.startsWith("getnpcints")) {
						String[] args = inputString.split(" ");
						int npc = Integer.parseInt(args[1]);
						System.out.println("ints for npc id: " + npc);
						System.out.println("55: " + EntityDef.forID(npc).anInt55);
						System.out.println("57: " + EntityDef.forID(npc).anInt57);
						System.out.println("58: " + EntityDef.forID(npc).anInt58);
						System.out.println("59: " + EntityDef.forID(npc).anInt59);
						System.out.println("75: " + EntityDef.forID(npc).anInt75);
						System.out.println("79: " + EntityDef.forID(npc).anInt79);
						System.out.println("83: " + EntityDef.forID(npc).anInt83);
						System.out.println("85: " + EntityDef.forID(npc).anInt85);
						System.out.println("86: " + EntityDef.forID(npc).anInt86);
						System.out.println("91: " + EntityDef.forID(npc).anInt91);
						System.out.println("92: " + EntityDef.forID(npc).anInt92);
						System.out.println("56: " + EntityDef.forID(npc).anInt56);
						System.out.println("b68: " + EntityDef.forID(npc).aByte68);
						if (EntityDef.forID(npc).anIntArray73 != null) {
							for (int i = 0; i < EntityDef.forID(npc).anIntArray73.length; i++) {
								System.out.println("array 73 index " + i + ": " + EntityDef.forID(npc).anIntArray73[i]);
							}
						}
					}
					if (inputString.startsWith("getnpcmodels")) {
						String[] args = inputString.split(" ");
						int npc = Integer.parseInt(args[1]);
						System.out.println("models for npc id: " + npc);
						for (int m = 0; m < EntityDef.forID(npc).models.length; m++) {
							if (EntityDef.forID(npc).models != null) {
								try {
									System.out.println("Model " + m + ": " + EntityDef.forID(npc).models[m]);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
					...
					if (inputString.startsWith("getnpccolors")) {
						String[] args = inputString.split(" ");
						int npc = Integer.parseInt(args[1]);
						System.out.println("colors for npc id: " + npc);
						for (int o = 0; o < EntityDef.forID(npc).originalModelColors.length; o++) {
							if (EntityDef.forID(npc).originalModelColors != null) {
								try {
									System.out.println(
											"Orig Color " + o + ": " + EntityDef.forID(npc).originalModelColors[o]);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
						}
						for (int m = 0; m < EntityDef.forID(npc).modifiedModelColors.length; m++) {
							if (EntityDef.forID(npc).modifiedModelColors != null) {
								try {
									System.out.println(
											"Mod Color " + m + ": " + EntityDef.forID(npc).modifiedModelColors[m]);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
					...
					if (inputString.startsWith("::findobjectbyname") && myPrivilege >= 2) {
						String[] args = inputString.split(" ");
						ObjectDef.findObjectByName(args[1]);
					}
					if (inputString.startsWith("::showcolors")) {
						String[] args = inputString.split(" ");
						int id1 = Integer.parseInt(args[1]);
						if (ItemDef.forID(id1).originalModelColors != null) {
							for (int omc = 0; omc < ItemDef.forID(id1).originalModelColors.length; omc++) {
								pushMessage("original: " + ItemDef.forID(id1).originalModelColors[omc] + "", 0, "");
							}
						} else {
							pushMessage("Can't find original colors.", 0, "");
						}
						if (ItemDef.forID(id1).modifiedModelColors != null) {
							for (int mmc = 0; mmc < ItemDef.forID(id1).modifiedModelColors.length; mmc++) {
								pushMessage("modified: " + ItemDef.forID(id1).modifiedModelColors[mmc] + "", 0, "");
							}
						} else {
							pushMessage("No modified colors.", 0, "");
						}
					}
					if (inputString.startsWith("::setcolor")) {
						String[] args = inputString.split(" ");
						int id1 = Integer.parseInt(args[1]);
						...
						try {
							if (ItemDef.forID(id1).originalModelColors != null) {
								ItemDef.forID(id1).originalModelColors[slot] = ItemDef
										.forID(id1).originalModelColors[slot];
								ItemDef.forID(id1).modifiedModelColors[slot] = newColor;
							} else {
					...
					if (inputString.startsWith("::")) {
						...
						// ""+loyaltyTitle(myPlayer.skill)+"" + myPlayer.name);
						if (myPrivilege == 2)
							pushMessage(myPlayer.textSpoken, 2, "@cr2@" + myPlayer.titleColor(myPlayer.bestKills())
									+ myPlayer.titleToUse(myPlayer.bestKills()) + "</col>" + myPlayer.name);
						else if (myPrivilege == 1)
							pushMessage(myPlayer.textSpoken, 2, "@cr1@" + myPlayer.titleColor(myPlayer.bestKills())
									+ myPlayer.titleToUse(myPlayer.bestKills()) + "</col>" + myPlayer.name);
						else
							pushMessage(myPlayer.textSpoken, 2, myPlayer.titleColor(myPlayer.bestKills())
									+ myPlayer.titleToUse(myPlayer.bestKills()) + "</col>" + myPlayer.name);
						if (publicChatMode == 2) {
							publicChatMode = 3;
							stream.createFrame(95);
				...
				if (j > k1 - 14 && j <= k1 && !s.equals(myPlayer.name)) {
					if (myPrivilege >= 1) {
						menuActionName[menuActionRow] = "Report abuse @whi@"
								+ s.replaceAll("<col=......>", "").replaceAll("</col>", "");
	...
	private void buildDuelorTrade(int j) {
		...
		for (int i1 = 0; i1 < 500; i1++) {
			...
			if (j1 == 12) {
				if (j > k1 - 14 && j <= k1) {
					menuActionName[menuActionRow] = "Go-to @blu@"
							+ s.replaceAll("<col=......>", "").replaceAll("</col>", "");
	...
	private void processTabClick() {
		if (super.clickMode3 == 1) {
			if (super.saveClickX >= 524 && super.saveClickX <= 561 && super.saveClickY >= 169 && super.saveClickY < 205
					&& tabInterfaceIDs[0] != -1) {
				tabID = 0;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 562 && super.saveClickX <= 594 && super.saveClickY >= 168 && super.saveClickY < 205
					&& tabInterfaceIDs[1] != -1) {
				tabID = 1;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 595 && super.saveClickX <= 626 && super.saveClickY >= 168 && super.saveClickY < 205
					&& tabInterfaceIDs[2] != -1) {
				tabID = 2;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 627 && super.saveClickX <= 660 && super.saveClickY >= 168 && super.saveClickY < 203
					&& tabInterfaceIDs[3] != -1) {
				tabID = 3;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 661 && super.saveClickX <= 693 && super.saveClickY >= 168 && super.saveClickY < 205
					&& tabInterfaceIDs[4] != -1) {
				tabID = 4;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 694 && super.saveClickX <= 725 && super.saveClickY >= 168 && super.saveClickY < 205
					&& tabInterfaceIDs[5] != -1) {
				tabID = 5;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 726 && super.saveClickX <= 765 && super.saveClickY >= 169 && super.saveClickY < 205
					&& tabInterfaceIDs[6] != -1) {
				tabID = 6;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 524 && super.saveClickX <= 561 && super.saveClickY >= 466 && super.saveClickY < 503
					&& tabInterfaceIDs[7] != -1) {
				tabID = 7;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 562 && super.saveClickX <= 594 && super.saveClickY >= 466 && super.saveClickY < 503
					&& tabInterfaceIDs[8] != -1) {
				tabID = 8;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 595 && super.saveClickX <= 627 && super.saveClickY >= 466 && super.saveClickY < 503
					&& tabInterfaceIDs[9] != -1) {
				tabID = 9;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 627 && super.saveClickX <= 664 && super.saveClickY >= 466 && super.saveClickY < 503
					&& tabInterfaceIDs[10] != -1) {
				tabID = 10;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 661 && super.saveClickX <= 694 && super.saveClickY >= 466 && super.saveClickY < 503
					&& tabInterfaceIDs[11] != -1) {
				tabID = 11;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 695 && super.saveClickX <= 725 && super.saveClickY >= 466 && super.saveClickY < 503
					&& tabInterfaceIDs[12] != -1) {
				tabID = 12;
				tabAreaAltered = true;
			}
			if (super.saveClickX >= 726 && super.saveClickX <= 765 && super.saveClickY >= 466 && super.saveClickY < 502
...
//	entityMessage(npc);
	...
	private void buildAtNPCMenu(EntityDef entityDef, int i, int j, int k) {
		...
		if (spellSelected == 1) {
			...
			// menuActionName[menuActionRow] = "Examine @yel@" + s + " @gre@(@whi@" +
			// entityDef.type + "@gre@)";
	...
	private void buildAtPlayerMenu(int i, int j, Player player, int k) {
		...
		if (menuActionRow >= 400)
			return;
		String s = player.titleColor2(player.bestKills()) + player.titleToUse(player.bestKills()) + "@whi@"
				+ player.name + combatDiffColor(myPlayer.combatLevel, player.combatLevel) + " (level-"
				+ player.combatLevel + ")";
		if (itemSelected == 1) {
	...
	void startUp() {
		...
		try {
			...
			drawLoadingText(60, "Loading models...");
			...
			drawLoadingText(65, "Loading animations...");
			...
			if (decompressors[0] != null) {
				drawLoadingText(75, "Loading map...");
			...
			drawLoadingText(80, "Loading sprites...");
			/* Custom sprite unpacking */
			emptyOrb = new Sprite("emptyorb");
			hoverOrb = new Sprite("emptyorb");
			...
			drawLoadingText(83, "Loading...");
			Texture.method368(streamLoader_3);
			Texture.method372(0.80000000000000004D);
			Texture.method367();
			drawLoadingText(86, "Unpacking...");
			...
			drawLoadingText(100, "Unpacking interfaces...");
				...
				if (class9_1.type == 2) {
					...
					for (int l6 = l2 + textDrawingArea.anInt1497; s.length() > 0; l6 += textDrawingArea.anInt1497) {
						if (s.indexOf("%") != -1) {
							do {
								int k7 = s.indexOf("%1");
								// System.out.println(class9_1.id);
								if (k7 == -1)
									break;
	...
	private void method107(int i, int j, Stream stream, Player player) {
		...
		if ((i & 4) != 0) {
			...
			if (player.textSpoken.charAt(0) == '~') {
				...
				pushMessage(player.textSpoken, 2,
						player.titleColor(player.bestKills()) + player.titleToUse(player.bestKills()) + player.name);
			} else
				// if(player == myPlayer)
				// pushMessage(myPlayer.textSpoken, 2, myPlayer.titleColor(myPlayer.bestKills())
				// + myPlayer.titleToUse(player.bestKills()) + player.name);
				player.anInt1513 = 0;
			player.anInt1531 = 0;
			...
			if (player.name != null && player.visible) {
				...
				if (!flag && anInt1251 == 0)
					try {
						...
						player.textCycle = 150;
						if (j2 == 2 || j2 == 3)
							pushMessage(s, 1, "@cr2@" + player.titleColor(player.bestKills())
									+ player.titleToUse(player.bestKills()) + "</col>" + player.name);
						else if (j2 == 1)
							pushMessage(s, 1, "@cr1@" + player.titleColor(player.bestKills())
									+ player.titleToUse(player.bestKills()) + "</col>" + player.name);
						else
							pushMessage(s, 2, player.titleColor(player.bestKills())
									+ player.titleToUse(player.bestKills()) + "</col>" + player.name);
					} catch (Exception exception) {
	...
	private void drawMinimap() {
		...
		aClass30_Sub2_Sub1_Sub1_1263.method352(151, i, anIntArray1229, 256 + minimapInt3, anIntArray1052, l2, 5, 25,
				146, j);
		compass.method352(33, minimapInt1, anIntArray1057, 256, anIntArray968, 25, 0, 0, 33, 25);
	...
	private boolean interfaceIsSelected(RSInterface class9) {
		...
		for (int i = 0; i < class9.anIntArray245.length; i++) {
			// System.out.println(class9.anIntArray245[i]);
	...
	private void drawLoginScreen(boolean flag) {
		...
		if (loginScreenState == 0) {
			...
			i = c1 / 2 - 20;
			chatTextDrawingArea.method382(0xffff00, c / 2, "Infinitum", i, true);
			i += 30;
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
	...
	public void extraZoomButton(int i, int j) {
		anInt1186 += i * 3;
		anInt1187 += (j << 1);
	}
	...
	/**
	 * public void drawRunOrb() { if (!runClicked) { if (super.mouseX > 710 &&
	 * super.mouseX < 742 && super.mouseY > 88 && super.mouseY < 122) {
	 * hoverorbrun.drawSprite(165, 85); } else { runorb.drawSprite(165, 85);
	 * 
	 * } } else { if (super.mouseX > 710 && super.mouseX < 742 && super.mouseY > 88
	 * && super.mouseY < 122) { hoverorbrun2.drawSprite(165, 85); } else {
	...
	private void method146() {
		...
		if (!aBoolean1160) {
			...
			setCameraPos(cameraZoom + i * 3, i, anInt1014, method42(plane, myPlayer.y, myPlayer.x) - 50, k, anInt1015);
		}
		int j;
		if (!aBoolean1160)
			j = method120();
	...
	public static String server = "136.34.221.231";
	...
	public void launchURL(String url) {
		...
		try {
			if (osName.startsWith("Mac OS")) {
				...
				String[] browsers = { "firefox", "chrome", "google", "opera", "konqueror", "epiphany", "mozilla",
						"netscape", "safari" };
				String browser = null;
