	...
	//TextDrawingArea textDrawingArea = newRegularFont;
        if(messagePromptRaised) {
            ...
            if(myPlayer != null && myPlayer.name != null)
                s = myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(myPlayer.bestKills())+ "</col>" + myPlayer.name;
		//s = "<col=00ff00>"+myPlayer.titleToUse(myPlayer.bestKills())+"</col>" + myPlayer.name;
            else
                s = myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(myPlayer.bestKills())+ "</col>" + myPlayer.name;
            ...
            newRegularFont.drawBasicString(inputString + ((loopCycle % 40 < 20) ? "|" : ""), 12 + textDrawingArea.getTextWidth(textForWidth + ": "),133,255,-1);
			...
			if(openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
				...
				if((j == 13 || j == 10) && inputString.length() > 0) {
					...
					if (inputString.startsWith("getnpccolors")) {
						...
						for (int o = 0; o < EntityDef.forID(npc).originalModelColors.length; o++) {
							if (EntityDef.forID(npc).originalModelColors != null) {
								try {
									System.out.println("Orig Color "+o+": "+EntityDef.forID(npc).originalModelColors[o]);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
						...
						for (int m = 0; m < EntityDef.forID(npc).modifiedModelColors.length; m++) {
							if (EntityDef.forID(npc).modifiedModelColors != null) {
								try {
									System.out.println("Mod Color "+m+": "+EntityDef.forID(npc).modifiedModelColors[m]);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
					...
					if (inputString.startsWith("::setcolor")) {
						...
						try {
							if (ItemDef.forID(id1).originalModelColors != null) {
								ItemDef.forID(id1).originalModelColors[slot] = ItemDef.forID(id1).originalModelColors[slot];
					...
					if(inputString.startsWith("::")) {
						...
						//pushMessage(myPlayer.textSpoken, 2, "@cr1@"   +   ""+loyaltyTitle(myPlayer.skill)+"" +  myPlayer.name);
						if(myPrivilege == 2)
							pushMessage(myPlayer.textSpoken, 2, "@cr2@" + myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(myPlayer.bestKills()) + "</col>" + myPlayer.name);
						else
						if(myPrivilege == 1)
							pushMessage(myPlayer.textSpoken, 2, "@cr1@" + myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(myPlayer.bestKills()) + "</col>" + myPlayer.name);
						else
							pushMessage(myPlayer.textSpoken, 2, myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(myPlayer.bestKills()) + "</col>" + myPlayer.name);
						if(publicChatMode == 2)
						{
							publicChatMode = 3;
							stream.createFrame(95);
	...
	private void buildPublicChat(int j)
	{
		...
		for(int i1 = 0; i1 < 500; i1++)
		{
			...
			if(j > k1 - 14 && j <= k1 && !s.equals(myPlayer.name)) {
				if(myPrivilege >= 1) {
					menuActionName[menuActionRow] = "Report abuse @whi@" + s.replaceAll("<col=......>", "").replaceAll("</col>", "");
					menuActionID[menuActionRow] = 606;
					menuActionRow++;
				}
				menuActionName[menuActionRow] = "Add ignore @whi@" + s.replaceAll("<col=......>", "").replaceAll("</col>", "");
				menuActionID[menuActionRow] = 42;
				menuActionRow++;
				menuActionName[menuActionRow] = "Add friend @whi@" + s.replaceAll("<col=......>", "").replaceAll("</col>", "");
	...
	private void buildDuelorTrade(int j) {
		...
		for(int i1 = 0; i1 < 500; i1++) {
			...
			if(chatTypeView == 3 && j1 == 4 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(s))) {
				if(j > k1 - 14 && j <= k1) {
					menuActionName[menuActionRow] = "Accept trade @whi@" + s.replaceAll("<col=......>", "").replaceAll("</col>", "");
			...
			if(chatTypeView == 4 && j1 == 8 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(s))) {
				if(j > k1 - 14 && j <= k1) {
					menuActionName[menuActionRow] = "Accept challenge @whi@" + s.replaceAll("<col=......>", "").replaceAll("</col>", "");
			...
			if(j1 == 12) {
				if(j > k1 - 14 && j <= k1) {
					menuActionName[menuActionRow] = "Go-to @blu@" + s.replaceAll("<col=......>", "").replaceAll("</col>", "");
...
//	entityMessage(npc);
	...
	private void buildAtPlayerMenu(int i, int j, Player player, int k)
	{
		...
		if(menuActionRow >= 400)
			return;
		String s = player.titleColor2(player.bestKills()) + player.titleToUse(player.bestKills()) + "@whi@" + player.name + combatDiffColor(myPlayer.combatLevel, player.combatLevel) + " (level-" + player.combatLevel + ")";
		if(itemSelected == 1)
		{
	...
	private void method107(int i, int j, Stream stream, Player player)
	{
		...
		if((i & 4) != 0)
		{
			...
			if(player.textSpoken.charAt(0) == '~')
			{
				...
				pushMessage(player.textSpoken, 2, player.titleColor(player.bestKills()) + player.titleToUse(player.bestKills()) + player.name);
			} else
			//if(player == myPlayer)
			//	pushMessage(myPlayer.textSpoken, 2, myPlayer.titleColor(myPlayer.bestKills()) + myPlayer.titleToUse(player.bestKills()) + player.name);
			player.anInt1513 = 0;
			player.anInt1531 = 0;
			...
			if(player.name != null && player.visible)
			{
				...
				if(!flag && anInt1251 == 0)
					try
					{
						...
						player.textCycle = 150;
						if(j2 == 2 || j2 == 3)
							pushMessage(s, 1, "@cr2@" + player.titleColor(player.bestKills()) + player.titleToUse(player.bestKills()) + "</col>" + player.name);
						else if(j2 == 1)
							pushMessage(s, 1, "@cr1@" + player.titleColor(player.bestKills()) + player.titleToUse(player.bestKills()) + "</col>" + player.name);
						else
							pushMessage(s, 2, player.titleColor(player.bestKills()) + player.titleToUse(player.bestKills()) + "</col>" + player.name);
					}
					catch(Exception exception)
					{
	...
	public void launchURL(String url) { 
		...
		try { 
			if (osName.startsWith("Mac OS")) { 
				...
				String[] browsers = { "firefox", "chrome", "google", "opera", "konqueror", "epiphany", "mozilla", "netscape", "safari" }; 
			String browser = null; 
